            ...
            match get_latest_version(&client, &name).await {
                Ok(new_version) => {
                    ...
                    Some(UpdateInfo {
                        ...
                        current_version,
                        new_version,
                        platform: Platform::Fdroid,
                    })
                }
                Err(e) => {
                    warn!("[F-droid] 检查 {} 失败: {}", name, e);
                    None
                }
            }
        });
...
/// 获取指定 F-droid 仓库的最新 release 版本号
///
...
async fn get_latest_version(
    ...
    let body: FdroidPackage = client
        .get(format!("https://f-droid.org/api/v1/packages/{}", app_name))
    ...
    Ok(body.version_code.to_string())
}
